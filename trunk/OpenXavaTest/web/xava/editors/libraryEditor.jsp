...
%>
	  <a href='<%=request.getContextPath()%>/xava/xfile?application=<%=applicationName%>&module=<%=module%>&fileId=<%=file.getId()%>&dif=<%=dif%>' 
...
%>
	<span valign='middle'>
	  <xava:image action='Library.choose' argv='<%="newLibraryProperty=" + Ids.undecorate(propertyKey)%>'/>
	  <%if(isEmpty) { %>			
		<xava:link action='Library.choose' argv='<%="newLibraryProperty=" + Ids.undecorate(propertyKey)%>'/>
	  <%} %>
	</span>   		
<%}%>
