...
<%@ page import="org.openxava.actions.OnChangeMyChartLabelColumnAction" %>
...
boolean showAllColumns = Boolean.TRUE.equals((Boolean) view.getRoot().getObject("xava.myChartColumnShowAllColumns"));
boolean acceptOnlyNumericColumns = "true".equalsIgnoreCase(request.getParameter("acceptOnlyNumericColumns"));
...
<%
	...
	for (java.util.Iterator it = columns.iterator(); it.hasNext(); ) {
			...
			if (acceptOnlyNumericColumns && !property.isNumber()) {
				continue;
			}
	...
	if (!showAllColumns) {%>
	<option value="<%=OnChangeMyChartLabelColumnAction.SHOW_MORE%>"><xava:message key="my_report_show_more_columns"/></option>
	<% } else {%>
	<option value="<%=OnChangeMyChartLabelColumnAction.SHOW_LESS%>"><xava:message key="my_chart_show_less_columns"/></option>
	<% }
